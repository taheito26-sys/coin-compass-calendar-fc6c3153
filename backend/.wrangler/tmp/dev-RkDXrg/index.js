...
function getAllowedOrigins(env2) {
  const raw2 = env2.ALLOWED_ORIGINS || "http://localhost:5173";
...
var assets_default = app;

// src/routes/prices.ts
var app2 = new Hono2();
app2.get("/", async (c) => {
  const raw2 = await c.env.PRICE_KV.get("prices:latest");
  if (!raw2) {
    return c.json({ prices: null, stale: true });
  }
  const snapshot = JSON.parse(raw2);
  const ageMs = Date.now() - snapshot.ts;
  return c.json({ prices: snapshot.prices, ts: snapshot.ts, ageMs, stale: ageMs > 6e5 });
});
app2.get("/history", async (c) => {
  const raw2 = await c.env.PRICE_KV.get("prices:history");
  const history = raw2 ? JSON.parse(raw2) : [];
  return c.json({ history, count: history.length });
});
var prices_default = app2;
...
async function authMiddleware(c, next) {
  ...
  try {
    ...
    if (payload.exp && typeof payload.exp === "number" && payload.exp < Date.now() / 1e3) {
...
__name(verifyHS256, "verifyHS256");
...
__name(b64UrlToBuffer, "b64UrlToBuffer");
...
app3.put("/:id", async (c) => {
  ...
  const EDITABLE_FIELDS = {
    timestamp: /* @__PURE__ */ __name((v) => String(v), "timestamp"),
    type: /* @__PURE__ */ __name((v) => String(v), "type"),
    qty: /* @__PURE__ */ __name((v) => Number(v), "qty"),
    unit_price: /* @__PURE__ */ __name((v) => Number(v), "unit_price"),
    fee_amount: /* @__PURE__ */ __name((v) => Number(v), "fee_amount"),
    fee_currency: /* @__PURE__ */ __name((v) => String(v), "fee_currency"),
    venue: /* @__PURE__ */ __name((v) => v == null ? null : String(v), "venue"),
    note: /* @__PURE__ */ __name((v) => v == null ? null : String(v), "note"),
    tags: /* @__PURE__ */ __name((v) => v == null ? null : JSON.stringify(v), "tags"),
    source: /* @__PURE__ */ __name((v) => String(v), "source")
  };
  const sets = [];
  const vals = [];
  ...
  const row = await c.env.DB.prepare("SELECT * FROM transactions WHERE id = ?").bind(txId).first();
...
var tracking_default = app4;

// src/routes/imported-files.ts
var app5 = new Hono2();
...
app5.post("/", async (c) => {
  ...
  const row = await c.env.DB.prepare("SELECT * FROM imported_files WHERE id = ?").bind(id).first();
  return c.json({ file: row }, 201);
});
var imported_files_default = app5;

// src/cron/poll-prices.ts
var BINANCE_TICKER = "https://api.binance.com/api/v3/ticker/24hr";
async function pollPrices(env2) {
  ...
  for (const asset of assets) {
    ...
    prices[asset.id] = {
      ...
      change_1h: null,
      // Binance 24hr ticker doesn't have 1h
  ...
  const snapshot = { prices, ts: now };
  await env2.PRICE_KV.put("prices:latest", JSON.stringify(snapshot), {
    expirationTtl: 600
    // 10 min
  });
  let history = [];
  try {
    const raw2 = await env2.PRICE_KV.get("prices:history");
  ...
  await env2.PRICE_KV.put("prices:history", JSON.stringify(history), {
    expirationTtl: 9e4
    // 25h
  });
  return { updated };
}
__name(pollPrices, "pollPrices");

// src/index.ts
var app6 = new Hono2();
app6.use("*", corsMiddleware);
app6.route("/api/assets", assets_default);
app6.route("/api/prices", prices_default);
app6.route("/api/transactions", transactions_default);
app6.route("/api/tracking-preferences", tracking_default);
app6.route("/api/imported-files", imported_files_default);
app6.get("/api/status", async (c) => {
  const raw2 = await c.env.PRICE_KV.get("prices:latest");
...
var src_default = {
  ...
  async scheduled(event, env2, ctx) {
    ctx.waitUntil(
      pollPrices(env2).catch(
        (err) => console.error("[cryptotracker] price poll failed:", err.message)
      )
    );
  }
};
...
// .wrangler/tmp/bundle-kMQiIv/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-kMQiIv/middleware-loader.entry.ts
