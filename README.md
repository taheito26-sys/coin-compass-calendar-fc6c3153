...
Cloudflare Pages reads `VITE_*` values at build time, so after changing env vars you must trigger a new deploy for the fix to take effect.
...
By default, only trusted Clerk script hosts (`cdn.jsdelivr.net`, `unpkg.com`, `cdn.clerk.com`) are accepted.

```env
VITE_CLERK_JS_URL=https://cdn.jsdelivr.net/npm/@clerk/clerk-js@6/dist/clerk.browser.js
# Optional: only if you intentionally host clerk.browser.js on your own domain
VITE_ALLOW_CUSTOM_CLERK_JS_URL=true
```
