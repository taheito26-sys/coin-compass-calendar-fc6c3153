...
function Set-WranglerSecret {
    ...
    $escapedValue = Escape-CmdValue $Value
    $backendPath = Join-Path $RepoPath "backend"
    Run-CmdLine -WorkingDirectory $backendPath -Command "echo $escapedValue| npx.cmd wrangler secret put $Name"
}

function Move-GoogleImportToTop {
...
function Patch-DashboardText {
    $path = Join-Path $RepoPath "src\pages\DashboardPage.tsx"
    if (-not (Test-Path $path)) {
        return
    }

    $content = Get-Content $path -Raw
    $content = $content -replace "Sign in to see your Supabase portfolio\.", "Sign in to sync your portfolio."
    $content = $content -replace "Not logged in", "Not signed in"

    $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    [System.IO.File]::WriteAllText($path, $content, $utf8NoBom)
}

function Patch-BackendTypes {
    $path = Join-Path $RepoPath "backend\src\types.ts"
    if (-not (Test-Path $path)) {
        return
    }
...
try {
    ...
    try {
        Run-CmdLine "git rebase --abort"
    } catch {}

    Write-Step "Create backup branch"
    $backupBranch = "backup-before-clerk-fix-" + (Get-Date -Format "yyyyMMdd-HHmmss")
    Run-CmdLine "git branch $backupBranch"

    Write-Step "Reset local main and create clean branch"
    Run-CmdLine "git fetch origin"
    Run-CmdLine "git switch main"
    Run-CmdLine "git reset --hard origin/main"
    Run-CmdLine "git switch -C $BranchName origin/main"

    Write-Step "Ignore local-only files"
    Ensure-GitignoreLine ".env.local"
    Ensure-GitignoreLine "backend/.dev.vars"
    Ensure-GitignoreLine "backend/.wrangler/"
    Ensure-GitignoreLine "_auth_patch_backup_*/"

    if (Test-Path ".\backend\.wrangler") {
        Remove-Item -Recurse -Force ".\backend\.wrangler" -ErrorAction SilentlyContinue
    }
    Run-CmdLine "git rm -r --cached --ignore-unmatch backend/.wrangler"

    Write-Step "Write frontend auth files"
...
'@
    ...
    Write-TextFile "src\hooks\usePortfolio.ts" @'
...
'@
    ...
    Write-Step "Write backend auth files"
    Write-TextFile "backend\src\middleware\auth.ts" @'
...
'@
    ...
    Write-Step "Write clean config files"
    Write-TextFile "backend\wrangler.toml" @'
...
'@
    ...
    Patch-DashboardText
    Move-GoogleImportToTop

    Write-Step "Write local env files"
    Write-TextFile ".env.local" @"
VITE_CLERK_PUBLISHABLE_KEY=$PublishableKey
VITE_WORKER_API_URL=$WorkerApiUrl
"@

    Write-TextFile "backend\.dev.vars" @"
CLERK_JWKS_URL=$ClerkJwksUrl
ALLOWED_ORIGINS=$AllowedOrigins
"@

    Write-Step "Install frontend dependencies"
    Run-CmdLine "npm.cmd uninstall @clerk/clerk-react"
    Run-CmdLine "npm.cmd install @clerk/react"
    Run-CmdLine "npm.cmd install"

    Write-Step "Install backend dependencies"
    Run-CmdLine -WorkingDirectory (Join-Path $RepoPath "backend") -Command "npm.cmd install"

    Write-Step "Build frontend"
    Run-CmdLine "npm.cmd run build"

    if (-not $SkipBackendDeploy) {
        Write-Step "Deploy backend and refresh secrets"
        Set-WranglerSecret -Name "CLERK_JWKS_URL" -Value $ClerkJwksUrl
        Set-WranglerSecret -Name "ALLOWED_ORIGINS" -Value $AllowedOrigins
        Run-CmdLine -WorkingDirectory (Join-Path $RepoPath "backend") -Command "npx.cmd wrangler deploy"
    }

    if (-not $SkipCommit) {
        Write-Step "Commit clean branch"
        Run-CmdLine "git add .gitignore .env.example backend/src backend/wrangler.toml backend/package.json backend/package-lock.json package.json package-lock.json src"
        Run-CmdLine "git add -u backend/.wrangler"
        Run-CmdLine 'git commit -m "Restore clean Clerk auth flow and Worker config"'
    }

    Write-Step "Done"
    Write-Host "Working branch: $BranchName" -ForegroundColor Green
    Write-Host "Backup branch: $backupBranch" -ForegroundColor Green
    Write-Host "Next: run npm run dev from the repo root." -ForegroundColor Green
}
finally {
    Pop-Location
}